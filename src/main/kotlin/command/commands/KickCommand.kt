package net.guneyilmaz0.bmo.command.commands

import net.dv8tion.jda.api.Permission
import net.dv8tion.jda.api.entities.Member
import net.dv8tion.jda.api.entities.Message
import net.dv8tion.jda.api.exceptions.InsufficientPermissionException
import net.dv8tion.jda.api.interactions.commands.OptionType
import net.dv8tion.jda.api.interactions.commands.build.Commands
import net.guneyilmaz0.bmo.command.ICommand
import net.guneyilmaz0.bmo.command.ICommandManager

class KickCommand(name: String, description: String) : ICommand(name, description) {

    init {
        usageMessage = "-kick <@user> [reason]"
        permission = Permission.KICK_MEMBERS
        permissionMessage = "You need the <permission> permission to use this command."
    }

    override fun execute(member: Member, message: Message, args: Array<String>) {
        if (!testPermission(message)) return

        val mentionedMember = message.mentions.members.firstOrNull()
        if (mentionedMember == null) {
            message.channel.sendMessage("Please mention a user to kick.").queue()
            return
        }

        if (!member.canInteract(mentionedMember)) {
            message.channel.sendMessage("You cannot kick this member.").queue()
            return
        }

        val reason = if (args.size > 1) args.drop(1).joinToString(" ") else "No reason provided"

        try {
            mentionedMember.kick().reason(reason).queue {
                message.channel.sendMessage(":boot: ${mentionedMember.effectiveName} has been kicked.").queue()
            }
        } catch (e: InsufficientPermissionException) {
            message.channel.sendMessage("I don't have permission to kick members.").queue()
        }
    }

    override fun createSlashCommand(name: String) {
        ICommandManager.guild?.upsertCommand(
            Commands.slash(name, description)
                .addOption(OptionType.USER, "user", "Kullanıcı", true)
                .addOption(OptionType.STRING, "reason", "Reason for kicking", false)
        )?.queue()
    }
}
